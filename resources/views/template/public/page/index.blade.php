...
<x-app-layout>
  ...
  <div class="md:flex p-0 md:flex-row-reverse {{ $bgColor }} {{ $textColor }}">
            ...
            @foreach($content->carousel_data['firstSlider'] as $key => $slide)
              ...
              <div class="swiper-slide mobile-item-square">
                @if(isset($slide['image']['size']))
                  ...
                  @else
                    <div class="relative h-full">
                      <div style="opacity:0;position:absolute;font-size:.1em;">{{ $key }}</div>
                      <picture class="block w-full h-full object-cover">
                        {!! generatePictureHtml($slide['image']['size']) !!}
                      </picture>
                      {{--                      <input type="hidden" data-id="firstSlider-{{ $key }}" class="json-image" value="{{ e(json_encode($slide['image']['size'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block w-full h-full object-cover">--}}
                ...
                @if(isset($slide['link'])&&!empty($slide['link'])&&isset($slide['button'])&&!empty($slide['button']))
                  <div class="absolute w-full z-10 bottom-12 left-0 p-4 text-white  text-center" style="padding-bottom:0;">
                    <x-public.primary-button href="{{ $slide['link'] }}" class="rounded-full border-white m-text-body d-text-body">Подробнее</x-public.primary-button>
                  </div>
                @endif
                @if(isset($slide['id'])&&$slide['id']=='timer')
                  <div>
                    <div id="timer-container" class="absolute w-full z-10 left-0 p-4 cormorantInfant text-center text-black" style="opacity:0;top:82%;transform: translateY(-50%);padding-bottom:0;">
                      <div id="timer" class="text-center flex justify-center space-x-2">
                        <div class="scale-text cormorantInfant flex text-[28px] items-center px-2.5 py-0.5">
                          {{--                            <div class="relative">--}}
                          ...
                          <div class="relative">
                            <div class="flex space-x-1">
                              <div class="time w-full flex justify-center items-center text-center digit" style="width: .4em;" id="hours-1">0</div>
                              <div class="time w-full flex justify-center items-center text-center digit" style="width: .4em;" id="hours-2">0</div>
                            </div>
                            <div id="hours-text" class="timer-text font-roboto_condensed text-center left-1/2 -translate-x-1/2 absolute bottom-[-5px]">часов</div>
                          </div>
                          <div class="font-foglihten_no06 inline-block mx-1">:</div>
                          <div class="relative">
                            <div class="flex space-x-1">
                              <div class="time w-full flex justify-center items-center text-center digit" style="width: .4em;" id="minutes-1">0</div>
                              <div class="time w-full flex justify-center items-center text-center digit" style="width: .4em;" id="minutes-2">0</div>
                            </div>
                            <div id="min-text" class="timer-text font-roboto_condensed text-center left-1/2 -translate-x-1/2 absolute bottom-[-5px]">минут</div>
                          </div>
                          <div class="font-foglihten_no06 inline-block mx-1">:</div>
                          <div class="relative">
                            <div class="flex space-x-1">
                              <div class="time w-full flex justify-center items-center text-center digit" style="width: .4em;" id="seconds-1">0</div>
                              <div class="time w-full flex justify-center items-center text-center digit" style="width: .4em;" id="seconds-2">0</div>
                            </div>
                            <div id="sec-text" class="timer-text font-roboto_condensed text-center left-1/2 -translate-x-1/2 absolute bottom-[-5px]">секунд</div>
                          </div>
                        </div>
                      </div>
                    </div>
    ...
    <div class="bg-myGreen flex-1 flex flex-col-reverse md:flex-col justify-between" @if(getSettings('winterMode')) style="color: #fff;background-color: #2C4B3A;background: url('{{ asset('img/top-bg.jpg?1') }}');background-size: cover;" @endif>
      <div class="p-0 md:pt-3 pb-6 mb:pb-12 md:pr-4 lg:pt-[98px] lg:pr-0 w-full bg-cover relative flex-1">
        @if(getSettings('springMode') && isset($content->image_data['firstImage']['size']))
          <input type="hidden" data-id="firstImage" class="json-image" value="{{ e(json_encode($content->image_data['firstImage']['size'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover absolute left-0 right-0 top-0 bottom-0 w-full h-full">
        @endif
        <div class="text-center md:text-left pt-3 md:pt-0 md:pl-4 lg:pl-16 relative z-10 @if(getSettings('springMode')) text-white @endif">
          <h1 class="m-headline-1 d-headline-1 mb-4 md:mb-6">{{ $content->text_data['headline1'] }}</h1>
          <p class="d-text-body m-text-body mb-4 md:mb-12 mobile-hidden-br md:px-0 px-2">{!! nl2br($content->text_data['subtitle1']) !!}</p>
          <a href="{{ route('page.dermatologists') }}" class="h-11 inline-flex items-center justify-center px-3 md:px-4 lg:px-7 border @if(getSettings('springMode')) border-white @else{{ $borderColor }}@endif text-xl leading-none font-medium md:w-full max-w-[357px] md:text-2xl md:h-14">Подробнее</a>
        </div>
      </div>
      ...
      <div class="w-full sm:w-auto lg:mx-0 flex justify-end">
        <div class="p-0 w-full bg-cover relative">
          @if(!getSettings('springMode') && isset($content->image_data['activeComponentsImage']['size']))
            <input type="hidden" data-id="activeComponentsImage" class="json-image" value="{{ e(json_encode($content->image_data['activeComponentsImage']['size'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover absolute left-0 right-0 top-0 bottom-0 w-full h-full">
          @endif
          <div class="backdrop-blur-sm text-center font-medium text-white text-lg xl:text-2xl pt-px sm:pt-6 px-1 sm:px-6 pb-3.5 z-10 relative @if(getSettings('springMode')) bg-springGreen @endif"
            ...
            <div
              class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[42px] h-[42px] xl:w-[58px] xl:h-[58px] observed inline hidden"></div>
              ...
              @php
                ...
                foreach ($lines as $key => $line) {
                  if (preg_match('/\[(\d+)%\]/', $line, $matches)) {
                      ...
                      $replacement = generateSVG($percentage, 58, 5, 'absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[42px] h-[42px] xl:w-[58px] xl:h-[58px] observed');
                      $replacement = '<div class="percent-round relative inline-block align-middle mx-3 xl:mx-4 leading-none -translate-y-1 text-19 xl:text-2xl font-bold"><div class="text-19 xl:text-2xl font-bold">'.$percentage.'%</div>'.$replacement.'</div>';
                ...
                echo $result;
              @endphp</div>
            <div class="italic leading-1.2">{{ $content->text_data['activeComponentsPercentage2'] ?? '' }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div id="swiper-block-categories" class="swiper">
      <div class="swiper-wrapper">
        <div class="swiper-slide flex flex-col items-center px-[13px] max-w-[170px] w-auto relative">
          ...
          @endif
          <div class="text-center text-xs md:text-[22px] mt-1.5 sm:mt-3 pb-1 w-16 md:w-[138px]">В наличии</div>
        </div>
        @foreach($addData->categories as $category)
          <div class="swiper-slide flex flex-col items-center px-[13px] max-w-[170px] w-auto relative">
            <a href="{{ route('catalog.category', $category->slug) }}" class="block absolute w-full h-full left-0 top-0 z-10"></a>
            ...
            @endif
            <div class="text-center text-xs md:text-[22px] mt-1.5 sm:mt-3 pb-1 w-16 md:w-[138px]">{{ $category->title }}</div>
          </div>
        @endforeach
        <div class="swiper-slide flex flex-col items-center px-[13px] max-w-[170px] w-auto relative">
          <a href="{{ route('product.vouchers') }}" class="block absolute w-full h-full left-0 top-0 z-10"></a>
          ...
          @endif
          <div class="text-center text-xs md:text-[22px] mt-1.5 sm:mt-3 pb-1 w-16 md:w-[138px]">Подарочные сертификаты</div>
        </div>
        <div class="swiper-slide flex flex-col items-center px-[13px] max-w-[170px] w-auto relative">
          <a href="{{ route('product.presents') }}" class="block absolute w-full h-full left-0 top-0 z-10"></a>
          @if(isset($content->image_data['presents']['size']))
          ...
          @endif
          <div class="text-center text-xs md:text-[22px] mt-1.5 sm:mt-3 pb-1 w-16 md:w-[138px]">Наши презенты</div>
        </div>
      </div>
    </div>
  </div>
  ...
  @if(isset($addData->weRecommend2))
    <div data-onload="swiperProductCarousel2" class="px-2 sm:px-4 md:px-8 lg:px-14 xl:px-16 py-12 md:py-14 lg:py-16 xl:py-[86px]">
      <div>
      ...
      function swiperProductCarousel2(){
        if(document.getElementById('swiper-product-carousel2')){
          ...
          function handleVisibleSlides() {
            const { activeIndex, params, slides } = this;
            const visibleSlides = [];
  ...
  @if(isset($addData->vigodno)&&$addData->vigodno->isNotEmpty())
    <div data-onload="swiperProductCarousel" class="px-2 sm:px-4 md:px-8 lg:px-14 xl:px-16 py-12 md:py-14 lg:py-16 xl:py-[86px]">
      <div>
        <div class="mb-6 md:mb-12">
      ...
      <div class="relative mx-auto max-w-[935px]">
        @if(isset($content->image_data['uniqueWorkingRecipes']['size']))
          <div class="item-square uniqueWorkingRecipes">
            <input type="hidden" data-id="uniqueWorkingRecipes" class="json-image" value="{{ e(json_encode($content->image_data['uniqueWorkingRecipes']['size'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover relative w-full min-h-[255px]" data-img-class="absolute left-0 top-0 sm:relative sm:left-auto sm:top-auto h-full w-full">
          </div>
        @endif
        <div
    ...
    <div class="flex-col flex md:flex-row-reverse justify-between md:-mx-3 lg:-mx-7">
      <div class="w-full lg:w-1/2 md:px-3 lg:px-7 relative md:mb-0 mb-5">
        @isset($content->text_data['dermatologists-img-text'])
          <div class="absolute z-10 left-5 sm:left-3 md:left-4 lg:left-8 xl:left-12 top-5 md:top-4 lg:top-8 xl:top-12 text-17 md:text-xl lg:text-2xl xl:text-28 italic !leading-1.6">{!! nl2br($content->text_data['dermatologists-img-text']) !!}</div>
        @endisset
        <div class="item-square dermgstImage">
          @if(isset($content->image_data['dermatologistsImg']['size']))
            ...
            <input type="hidden" data-id="new_products_line" class="json-image" value="{{ e(json_encode($content->image_data['dermatologistsImg']['size'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover relative" data-img-class="h-full w-full">
          @endif
        </div>
      </div>
      <div class="w-full lg:w-1/2 md:px-3 lg:px-7">
        <div class="bg-myGreen !leading-1.6 md:text-xl lg:text-2xl xl:text-32 m-text-body text-center mb-11 pt-7 pb-6 px-2 sm:px-4 md:px-8 lg:px-14 xl:px-16 -mx-2 sm:-mx-4 md:m-0 flex flex-col justify-center md:justify-between md:py-20 h-full">
          <div class="lh-outline-none">
            <a @if(!auth()->check()) href="javascript:;" data-src="#dermatologists-modal" data-fancybox-no-close-btn @else href="https://t.me/dermatolog_lm_bot" @endif class="uppercase underline underline-offset-4 hover:no-underline outline-none">проконсультироваться с <br/>врачом-дерматологом</a>
          </div>
    ...
    <div class="flex-col md:flex-row flex items-center justify-between">
      <div class="hidden md:block md-item-square w-full md:max-w-[39.4817%] md:mb-0 mb-5">
        @if(isset($content->image_data['new_products_line']['size']))
          <input type="hidden" data-id="new_products_line" class="json-image" value="{{ e(json_encode($content->image_data['new_products_line']['size'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover max-h-[255px] sm:max-h-none relative" data-img-class="max-h-[255px] sm:max-h-none h-full w-full">
        @endif
  ...
  @if(isset($content->text_data['checklist']['file']))
    <x-public.popup id="pdf-alert">
      ...
      <div class="m-text-body d-text-body text-center">
        ...
        <div class="mb-8">
          <x-public.green-button href="{{ $content->text_data['checklist']['file'] }}" target="_blank" onclick="Fancybox.close()">
            Смотреть
          </x-public.green-button>
        </div>
        <a href="#" id="hidden-alert" class="text-myGreen2 underline hover:no-underline underline-offset-4">Больше не показывать</a>
      </div>
    </x-public.popup>
    <script>
      window.checklist = {
