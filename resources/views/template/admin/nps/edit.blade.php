...
<x-admin-layout>
  ...
  <form action="{{ route('admin.nps.update', $survey->slug) }}" method="post">
    @method('PUT')
    @csrf
    <div class="border-b">
      ...
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          <div class="form-group">
            ...
            <x-text-input type="text" name="name" id="name" value="{{ old('name') ?? $survey->name }}" class="mt-1 block w-full" required />
          </div>
          <div class="form-group">
            <x-input-label for="slug" :value="__('Текст ссылки')" />
            <x-text-input type="text" name="slug" id="slug" value="{{ old('slug') ?? $survey->slug }}" class="mt-1 block w-full" required />
        ...
        <div class="carousel-contructor mb-5">
          ...
          <div class="carousel space-y-3 mb-1 p-2 border-gray-300 rounded-md border">
            @foreach($survey->questions as $question)
              <x-admin.content.home-slider-1>
                <x-text-input type="hidden" data-name="id" class="mt-1 block w-full" name="carousel_data[surveyQuestions][{{ $question->order }}][id]" value="{{ $question->id }}"/>
                <div class="form-group">
                  <x-input-label :value="__('Текст вопроса')" />
                  <x-text-input type="text" name="carousel_data[surveyQuestions][{{ $question->order }}][text]" id="carousel_data-surveyQuestions-{{ $question->order }}-text" data-name="text" class="mt-1 block w-full" value="{{ $question->text }}"/>
                  <div class="hint">Скрытое системное поле</div>
                </div>
                <div class="form-group">
                  <x-input-label :value="__('Комментарий')" />
                  <x-text-input type="text" name="carousel_data[surveyQuestions][{{ $question->order }}][comment_text]" id="carousel_data-surveyQuestions-{{ $question->order }}-comment_text" data-name="comment_text" class="mt-1 block w-full" value="{{ $question->comment_text }}"/>
                  <div class="hint">Скрытое системное поле</div>
                </div>
                <div class="form-group flex items-center">
                  <x-checkbox name="carousel_data[surveyQuestions][{{ $question->order }}][is_hidden]" id="carousel_data-surveyQuestions-{{ $question->order }}-is_hidden" data-name="is_hidden" value="1"  :checked="($question->is_hidden ?? false) ? true : false" />
                  <x-input-label class="ml-2" :value="__('Скрыть вопрос')"/>
                </div>
              </x-admin.content.home-slider-1>
            @endforeach
