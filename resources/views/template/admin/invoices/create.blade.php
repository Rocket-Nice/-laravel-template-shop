...
<x-admin-layout>
  ...
  <form action="{!! route('admin.invoices.store') !!}{{ stristr($_SERVER['REQUEST_URI'], '?') }}" method="post">
    @csrf
    <div class="border-b">
    ...
    <div class="mx-auto px-2 sm:px-3 lg:px-4 py-6" id="tab-content">
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          <div class="form-group"><p>Выбрано {{ denum($orders, ['%d заказ','%d заказа','%d заказов']) }}</p></div>
          <div class="form-group">
            <x-input-label for="name" :value="__('Наименование накладной')" />
            <x-text-input type="text" name="name" id="name" placeholder="Введите наименование накладной" value="{{ old('name') ?? '' }}" class="mt-1 block w-full" required />
          </div>
        </div>
      </div>
    </div>
    <div class="px-3 sm:px-4 lg:px-6 py-6 flex justify-end">
      <x-primary-button>Создать накладную</x-primary-button>
    </div>
  </form>
</x-admin-layout>
