...
<x-admin-layout>
  ...
  <form action="{{ route('admin.product_types.update', $product_type->id) }}" method="post">
    @csrf
    @method('PUT')
    <div class="border-b">
      ...
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          <div class="form-group">
            <x-input-label for="product_type-name" :value="__('Наименование')" />
            <x-text-input type="text" name="name" id="product_type-name" value="{{ old('name') ?? $product_type->name }}" class="mt-1 block w-full" required />
          </div>
        </div>
      </div>
    </div>
