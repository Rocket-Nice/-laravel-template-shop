...
<x-admin-layout>
  ...
  <form action="{{ route('admin.product-group.store') }}" method="post">
    @csrf
    <div class="mx-auto px-3 sm:px-4 lg:px-6 py-6">
      <div class="sm:w-[75%]">
        ...
        <div class="form-group">
          ...
          <x-textarea name="description" id="description" class="mt-1 block w-full">{{ old('description') }}</x-textarea>
        </div>
        <div class="form-group">
          ...
          <select name="content[]" id="content" multiple class="multipleSelect form-control">
            @foreach($contents as $content)
              <option value="{{ $content->id }}" data-keywords="{{ $content->route }}" @if(old('content')&&in_array($content->id, old('content'))){!! 'selected' !!}@endif>{{ $content->id }}: {{ $content->title }} ({{ $content->route }})</option>
            @endforeach
          </select>
        </div>
        <div class="form-group">
                  ...
                  <tr>
                    ...
                    <td class="border p-px text-center">
                      <label :for="product.id+'order'">
                        <input type="text" :name="'products['+product.id+'][order]'" class="numeric-field bg-transparent p-0 border-0 w-full text-center text-sm" :id="product.id+'order'" :value="product.order">
                      </label>
                    </td>
