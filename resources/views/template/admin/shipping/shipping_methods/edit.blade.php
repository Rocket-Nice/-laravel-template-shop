      ...
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          <div class="form-group">
            ...
            <x-text-input type="text" name="code" id="code" value="{{ old('code') ?? $shippingMethod->code }}" class="mt-1 block w-full" required />
          </div>
          <div class="form-group">
            <x-input-label for="name" :value="__('Наименование')" />
            <x-text-input type="text" name="name" id="name" value="{{ old('name') ?? $shippingMethod->name }}" class="mt-1 block w-full" required />
          </div>
          <div class="form-group">
            <x-input-label for="add_price" :value="__('Добавлять к стоимости доставки')" />
            <x-text-input type="text" name="add_price" id="add_price" value="{{ old('add_price') ?? $shippingMethod->add_price }}" class="mt-1 block w-full numeric-fields" />
          </div>

          <div class="form-group">
            <div class="flex items-center">
              <x-checkbox id="active" name="active" value="1"
                          :checked="(old('active') ?? $shippingMethod->active) ? true : false"/>
