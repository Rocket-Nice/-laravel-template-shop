...
<x-admin-layout>
  <x-slot name="header">
    ...
    <div class="text-center">
      <a href="{{ route('admin.roles.create') }}" class="button button-success m-2">Добавить роль</a>
    </div>
  </x-slot>
  <div class="mx-auto">
    <div class="relative overflow-x-auto min-h-[500px]">
      <table class="table-auto w-full text-center border-collapse border border-gray-200 rounded-md text-sm">
        ...
        <tr>
          ...
          <th class="bg-gray-100 border p-2">Разрешения</th>
          <th class="bg-gray-100 border p-2" style="width:5%"></th>
        </tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <td class="border p-2">
              @foreach($role->permissions as $permission)
                <span class="badge-blue text-xs">{{ $permission->name }}</span>
              @endforeach
            </td>
            <td class="border p-2 text-center">
              <a class="button button-light-secondary button-sm" href="{{ route('admin.roles.edit', ['role' => $role->id]) }}">
        ...
        @empty
          <tr>
            <td colspan="7">
              <div class="text-gray-400 text-2xl p-5 text-center">Нет ролей</div>
            </td>
          </tr>
        @endforelse
        </tbody>
    ...
    <div class="p-2">
      {{ $roles->appends(request()->input())->links('pagination::tailwind') }}
    </div>
  </div>
</x-admin-layout>
