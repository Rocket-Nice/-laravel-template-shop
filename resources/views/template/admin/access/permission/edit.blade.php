      ...
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          <div class="form-group">
            ...
            <x-text-input type="text" name="name" id="name" value="{{ old('name') ?? $permission->name }}" class="mt-1 block w-full" required />
          </div>
          <div class="form-group">
            <x-input-label for="roles" :value="__('Роль')" />
            <select id="role" name="roles[]" multiple class="multipleSelect form-control">
              <option value="">Выбрать</option>
              @foreach($roles as $role)
                <option value="{{ $role->id }}" @if($permission->roles()->where('id', $role->id)->first()){!! 'selected' !!}@endif>{{ $role->name }}</option>
              @endforeach
            </select>
          </div>
        </div>
