...
<x-admin-layout>
  ...
  <form action="{{ route('admin.cat-in-bag.categories.update', $category->id) }}" method="post">
    @csrf
    @method('PUT')
    <div class="border-b">
          ...
          <div class="form-group">
            ...
            <div class="flex items-end space-x-3 mb-1 p-2 border-gray-300 rounded-md border">
              <div id="lfm-preview-image" class="rounded-md overflow-hidden w-20" data-name="image">
                @if(old('image') || isset($category->data['image']))
                  <input type="hidden" name="image[img]" id="input-lfm-image" value="{{ old('image')['img'] ?? $category->data['image']['img'] ?? '' }}">
                  <input type="hidden" name="image[thumb]" id="input-lfm-thumb-image" value="{{ old('image')['thumb'] ?? $category->data['image']['thumb'] ?? $category->data['image']['img'] ?? '' }}">
                  <a href="javascript:;" data-fancybox="true" data-src="{{ old('image')['img'] ?? $category->data['image']['img'] ?? '' }}" style="display: block;">
                    <img src="{{ old('image')['thumb'] ?? $category->data['image']['thumb'] ?? $category->data['image']['img'] ?? '' }}" class="overflow-hidden max-w-full object-cover object-center">
                  </a>
                @endif
              </div>
              <div class="mb-2">
          ...
          <div class="form-group">
            <div class="flex items-center">
              <x-checkbox id="is_enabled" name="is_enabled" value="1"
                          :checked="old('is_enabled', $category->is_enabled) ? true : false"/>
