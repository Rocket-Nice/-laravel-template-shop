...
<x-admin-layout>
  ...
  @php
    ...
    $selectedProductId = old('product_id', $prize->product_id);
  ...
  <form action="{{ route('admin.cat-in-bag.prizes.update', $prize->id) }}" method="post">
    @csrf
    @method('PUT')
    <div class="border-b">
      ...
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          <div class="form-group">
            ...
            <div class="flex items-end space-x-3 mb-1 p-2 border-gray-300 rounded-md border">
              <div id="lfm-preview-prizeImage" class="rounded-md overflow-hidden w-20" data-name="image">
                @if(old('image') || isset($prize->data['image']))
                  <input type="hidden" name="image[img]" id="input-lfm-image" value="{{ old('image')['img'] ?? $prize->data['image']['img'] ?? '' }}">
                  <input type="hidden" name="image[thumb]" id="input-lfm-thumb-image" value="{{ old('image')['thumb'] ?? $prize->data['image']['thumb'] ?? $prize->data['image']['img'] ?? '' }}">
                  <a href="javascript:;" data-fancybox="true" data-src="{{ old('image')['img'] ?? $prize->data['image']['img'] ?? '' }}" style="display: block;">
                    <img src="{{ old('image')['thumb'] ?? $prize->data['image']['thumb'] ?? $prize->data['image']['img'] ?? '' }}" class="overflow-hidden max-w-full object-cover object-center">
                  </a>
                @endif
              </div>
              <div class="mb-2">
          ...
          <div class="form-group">
            ...
            <x-text-input type="number" name="total_qty" id="total_qty" value="{{ old('total_qty') ?? $prize->total_qty }}" class="mt-1 block w-full" min="0" required />
          </div>
          <div class="form-group">
            ...
            <select id="category_id" name="category_id" class="form-control w-full" required>
              ...
              @foreach($categories as $category)
                <option value="{{ $category->id }}" @if((old('category_id') ?? $prize->category_id) == $category->id) selected @endif>{{ $category->name }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select id="product_id" name="product_id" class="multipleSelect form-control w-full" data-keywords="1" required>
              ...
              @foreach($products as $product)
                <option value="{{ $product->id }}" data-keywords="{{ $product->sku }}" data-type="{{ $product->type_id }}" @if((old('product_id') ?? $prize->product_id) == $product->id) selected @endif>{{ $product->name }} ({{ $product->sku }})</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
            <div class="flex items-center">
              <x-checkbox id="is_enabled" name="is_enabled" value="1"
                          :checked="old('is_enabled', $prize->is_enabled) ? true : false"/>
          ...
          <div class="form-group">
            <div class="flex items-center">
              <x-checkbox id="is_golden" name="is_golden" value="1"
                          :checked="old('is_golden', $prize->is_golden) ? true : false"/>
          ...
          <div class="form-group">
            <div class="flex items-center">
              <x-checkbox id="is_certificate" name="is_certificate" value="1"
                          :checked="old('is_certificate', $prize->is_certificate) ? true : false"/>
