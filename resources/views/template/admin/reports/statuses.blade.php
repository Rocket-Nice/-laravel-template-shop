...
{{--    </a>--}}
    <a href="{{ route('admin.reports.export', ['report' => 'order-statuses']) }}&{{ parse_url($_SERVER['REQUEST_URI'], PHP_URL_QUERY) ?? '' }}" class="block bg-gray-200 rounded-md p-1">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
  ...
  <div class="w-full mt-4 mx-auto flex flex-wrap gap-4" id="statuses-cards"></div>
  <script>
    (async () => {
      const res = await fetch('/admin/reports/statuses-data' + window.location.search);
      const rows = await res.json();
      const container = document.getElementById('statuses-cards');
      container.innerHTML = '';

      Object.entries(rows).forEach(([key, { total, status, color, status_key }]) => {
        const displayStatus = status || 'Без статуса';
        const bgColor = color || 'bg-gray-200';

        container.innerHTML += `
            <div class="w-52 min-h-[110px] rounded-xl shadow-md flex flex-col items-center justify-center p-4 ${bgColor}">
                <a href="{{ route('admin.orders.index') }}?status[]=${status_key}" class="text-base text-gray-700 font-medium mb-2 text-center flex items-center gap-1">
                    ${displayStatus}
                </a>
