<x-admin-layout>
  ...
  <x-admin.search-form :route="url()->current()">
    ...
    <div class="p-1 w-full lg:w-1/2">
        ...
        <div class="form-group">
          ...
          <select id="category_id" name="category_id" class="multipleSelect form-control w-full">
            ...
            @foreach($categories as $category)
              <option value="{{ $category->id }}" @if(request()->get('category_id')==$category->id){!! 'selected' !!}@endif>@if($category->parent){{ $category->parent->title }} > @endif{{ $category->title }}</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="p-1 w-full lg:w-1/2">
        <div class="form-group">
          <x-input-label for="category_id" :value="__('Подарок')" />
          <select class="form-control" name="is_gift" style="width: 100%;">
            <option value="0" @if(!request()->get('is_gift')){!! 'selected' !!}@endif>Все</option>
            <option value="1" @if(request()->get('is_gift')&&request()->get('is_gift')=='1'){!! 'selected' !!}@endif>Без подарков</option>
            <option value="2" @if(request()->get('is_gift')&&request()->get('is_gift')=='2'){!! 'selected' !!}@endif>Только подарки</option>
          </select>
        </div>
      </div>
  </x-admin.search-form>
...
{{--    </a>--}}
    <a href="{{ route('admin.reports.export', ['report' => 'top-products']) }}&{{ parse_url($_SERVER['REQUEST_URI'], PHP_URL_QUERY) ?? '' }}" class="block bg-gray-200 rounded-md p-1">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
  ...
  <div class="text-sm rounded-xl border border-gray-400 bg-gray-200 p-4 mt-4">
    <ul>
      <li class="font-bold">Всего товаров: <span class="font-normal total_qty"></span></li>
      <li class="font-bold">Общая сумма: <span class="font-normal total_amount"></span></li>
    </ul>
  </div>
  ...
  <script>
    ...
    (async () => {
      const res  = await fetch('/admin/reports/products-data' + window.location.search);
      const rows = await res.json();

      const tbody = document.querySelector('#statsTable tbody');

      Object.entries(rows.stats).forEach(([key, { product_name, total_qty, total_amount }]) => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
      <td class="border p-2">${product_name}</td>
      <td class="border p-2">${fmtNum(total_qty)}</td>
      <td class="border p-2">${fmtNum(total_amount)}</td>`;
        tbody.appendChild(tr);
      });
      document.querySelector('.total_qty').textContent = fmtNum(rows.total_qty);
      document.querySelector('.total_amount').textContent = `${fmtNum(rows.total_amount)} ₽`;
    })();
  </script>
</x-admin-layout>
