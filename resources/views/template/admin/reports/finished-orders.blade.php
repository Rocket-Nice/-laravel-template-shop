<x-admin-layout>
  ...
  <div class="flex justify-end space-x-2">
    ...
    <a href="{{ route('admin.reports.export', ['report' => 'finished-orders']) }}&{{ parse_url($_SERVER['REQUEST_URI'], PHP_URL_QUERY) ?? '' }}" class="block bg-gray-200 rounded-md p-1">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
  ...
  <div class="text-sm rounded-xl border border-gray-400 bg-gray-200 p-4 mt-4">
    <ul>
      ...
      <li class="font-bold">Всего завершенных заказов: <span class="font-normal total_finished"></span></li>
    </ul>
  </div>
  <div class="w-full mx-auto">
    ...
    <table id="statsTable" class="min-w-full w-full text-sm text-left mt-6 border">
      ...
      <tr>
        ...
        <th class="bg-gray-100 border p-2">Завершенные заказы</th>
      </tr>
      </thead>
      <tbody></tbody>
    </table>
  ...
  <script>
    ...
    (async () => {
      const res  = await fetch('/admin/reports/finished-orders-data' + window.location.search);
      ...
      const orderCount   = rows.map(r => Number(r.total_count_diff));
      const finishedOrderCount   = rows.map(r => Number(r.status_count_diff));
      ...
      <td class="border p-2">${fmtNum(finishedOrderCount[i])}</td>`;
        tbody.appendChild(tr);
        document.querySelector('.total_orders').textContent = fmtNum(r.total_orders);
        document.querySelector('.total_finished').textContent = fmtNum(r.total_finished);
      });
    })();
  </script>
</x-admin-layout>
