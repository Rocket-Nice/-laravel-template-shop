<x-admin-layout>
  ...
  <div class="flex justify-end space-x-2">
    ...
    <a href="{{ route('admin.reports.export', ['report' => 'new-clients']) }}&{{ parse_url($_SERVER['REQUEST_URI'], PHP_URL_QUERY) ?? '' }} class="block bg-gray-200 rounded-md p-1">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
  ...
  <div class="text-sm rounded-xl border border-gray-400 bg-gray-200 p-4 mt-4">
    <ul>
      <li class="font-bold">Всего новых клиентов: <span class="font-normal total_users"></span></li>
    </ul>
  </div>
  <div class="w-full mx-auto">
    ...
    <table id="statsTable" class="min-w-full w-full text-sm text-left mt-6 border">
      ...
      <tr>
        ...
        <th class="bg-gray-100 border p-2">Новые клиенты</th>
      </tr>
      </thead>
      <tbody></tbody>
    </table>
  ...
  <script>
    ...
    (async () => {
      const res  = await fetch('/admin/reports/new-users-data' + window.location.search);
      ...
      const newUsers   = rows.map(r => Number(r.new_user_count_diff));
      ...
      <td class="border p-2">${fmtNum(newUsers[i])}</td>`;
        tbody.appendChild(tr);
        document.querySelector('.total_users').textContent = fmtNum(r.total_users);
      });
    })();
  </script>
</x-admin-layout>
