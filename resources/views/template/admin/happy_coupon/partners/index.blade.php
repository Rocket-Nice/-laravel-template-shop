...
<x-admin-layout>
  ...
  <div class="hidden">
    <div id="partners-cabinet-settings" class="w-full max-w-2xl" style="display: none;">
      <form action="{{ route('admin.partners.settings') }}" method="POST" class="p-4">
        @csrf
        <div class="form-group">
          <x-input-label for="partnersDate" :value="__('Дата отображения заказов')"/>
          <x-text-input type="text" name="partnersDate" id="partnersDate" value="{{ toDatepickerFormat(getSettings('partnersDate')) }}" data-timepicker="0" data-minDate="false" placeholder="{{ now()->startOfMonth()->format('d.m.Y H:i') }}" class="mt-1 block w-full datepicker" required/>
        </div>
              ...
              <x-dropdown_menu>
                <x-slot name="content">
                  <div class="py-1" role="none">
                    ...
                    <a href="{{ route('admin.partners.statistic', $partner->slug) }}" class="hover:bg-gray-100 text-gray-700 block px-4 py-2 text-sm no-underline" role="menuitem" tabindex="-1">Статистика</a>
                    @if(auth()->user()->hasPermissionTo('Доступ в партнерский кабинет'))
