      ...
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          ...
          <div class="form-group">
            ...
            <x-text-input type="text" name="total" id="total" value="{{ old('total') }}" class="mt-1 block w-full" />
          </div>
          <div class="form-group">
            ...
            <select id="product" name="product_id" class="form-control w-full">
              ...
              @foreach($products as $product)
                <option value="{{ $product->id }}" @if(old('product_id') == $product->id){!! 'selected' !!}@endif>{{ $product->name }} ({{ $product->sku }})</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
