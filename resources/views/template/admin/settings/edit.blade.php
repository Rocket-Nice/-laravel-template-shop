...
<x-admin-layout>
  ...
  <form action="{{ route('admin.sytstem_settings.update', $setting->id) }}" method="post">
    @csrf
    @method('PUT')
    <div class="mx-auto px-3 sm:px-4 lg:px-6 py-6">
      <div class="sm:w-[75%]">
        <div class="form-group">
          ...
          <x-text-input type="text" name="key" id="key" value="{{ old('key') ?? $setting->key }}" class="mt-1 block w-full" required/>
        </div>
        <div class="form-group">
          <x-input-label for="value" :value="__('Значение')" />
          <x-textarea name="value" id="value" class="mt-1 tinymce-textarea w-full"  data-toolbar="undo redo | bold align bullist forecolor | link unlink code">{{ old('value') ?? $setting->value }}</x-textarea>
        </div>
        <div class="form-group">
