...
<x-admin-layout>
  <x-slot name="header">
    @if(isset($seo['title']))
      ...
      <a href="{{ route('admin.categories.create') }}" class="button button-success">Добавить категорию</a>
    @endif
  </x-slot>
  <x-admin.search-form :route="route('admin.categories.index')">
        ...
        @foreach($categories as $category)
          <tr>
            ...
            <td class="border p-2 text-left" style="max-width:220px">
              @if($category->parent)<span class="text-slate-400">{{ $category->parent->title }} ></span> @endif{{ $category->title }} ({{ $category->products_count }})
            </td>
            <td class="border p-2 text-left" style="max-width:220px">
              {!! $category->hidden ? '<span class="badge-red text-xs">Скрыт</span>' : '<span class="badge-green text-xs">Показан</span>' !!}
            </td>
            ...
            <td class="border p-2 text-right">
              <a class="button button-light-secondary button-sm" href="{{ route('admin.categories.edit', $category->slug) }}">
