...
<x-admin-layout>
  ...
  <form action="{{ route('admin.mailing_lists.update', $mailingList->id) }}" method="post">
    @csrf
    @method('PUT')
    <div class="border-b">
      ...
      <div id="tab-1-content" role="tabpanel">
        <div class="sm:w-[50%]">
          <div class="form-group">
            ...
            <x-text-input type="text" name="name" id="name" value="{{ old('name') ?? $mailingList->name }}" class="mt-1 block w-full" required />
          </div>
          <div class="form-group">
            <x-input-label for="method" :value="__('Способ рассылки')"/>
            <select id="method" name="method" class="form-control w-full">
              <option value="">Выбрать</option>
              <option value="SMS" @if($mailingList->method=='SMS'){!! 'selected' !!}@endif>SMS</option>
              <option value="Telegram" @if($mailingList->method=='Telegram'){!! 'selected' !!}@endif>Telegram</option>
            </select>
          </div>
          <div class="form-group">
            <x-input-label for="message" :value="__('Сообщение')" />
            <x-textarea name="message" id="message" class="mt-1 block w-full tinymce-textarea">{{ old('message') ?? $mailingList->message }}</x-textarea>
          </div>

          <div class="form-group">
            <x-input-label for="sending_date" :value="__('Дата проведения расылки')" />
            <x-text-input type="text" name="sending_date" id="sending_date" value="{{ old('sending_date') ?? ($mailingList->sending_date ? $mailingList->sending_date->format('d.m.Y H:i') : '') }}" data-minDate="false" class="mt-1 block w-full datepicker" />
          </div>
        </div>
      </div>
    </div>
