@props(['product' => null])
@php($getId = getCode(3))
<div class="{{ $attributes['class'] ?? 'w-1/2 lg:w-1/3 px-2 md:px-3 py-2 md:py-3 flex flex-col justify-between' }}">
  <div>
    <div>
      <div class="swiper product-item-swiper">
        <div class="swiper-wrapper product-item-swiper__wrapper">
          <div class="swiper-slide product-item-swiper__slide">
            <div class="product-card_item relative">
              @if(getSettings('puzzlesStatus') && $product->puzzles && $product->puzzles_count && is_numeric($product->puzzles_count))
                <div class="flex items-center absolute z-10 text-myGreen2 text-md" style="left: 9.625668449%; top:8.771929825%; color: @if($product->puzzle_color){{ $product->puzzle_color }}@else{{ '#6C715C' }}@endif">
                  <svg style="width: 6.417112299%;" viewBox="0 0 24 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path opacity="0.64" fill-rule="evenodd" clip-rule="evenodd" d="M0.00221464 21.1217C-0.000738212 21.2221 -0.000738212 21.2989 0.00221464 21.3993C0.0619892 22.749 0.160797 24.0623 0.307682 25.3299C0.367037 25.8422 0.434241 26.347 0.509893 26.8438C0.533726 27.0003 0.558397 27.156 0.583926 27.3109H0.586878C2.45013 27.0038 4.42853 26.8 6.49553 26.7085H6.77309C8.22589 26.7351 9.39227 27.3906 9.3775 28.1731C9.37455 28.368 9.29778 28.5422 9.16195 28.7105C8.90505 29.0265 8.76627 29.3926 8.76627 29.7854C8.76627 31.0108 10.16 32 11.8815 32C13.603 32 14.9968 31.0078 14.9968 29.7854C14.9968 29.3897 14.858 29.0265 14.6011 28.7105C14.4653 28.5452 14.3885 28.368 14.3855 28.1731C14.3708 27.3906 15.5371 26.7351 16.9929 26.7085C17.0962 26.7055 17.1966 26.7085 17.297 26.7115C19.3522 26.8 21.3218 27.0185 23.1821 27.3109C23.2137 27.1187 23.2441 26.9253 23.2733 26.7306C23.3421 26.2704 23.4039 25.8033 23.459 25.3299C23.6065 24.0622 23.7059 22.749 23.7638 21.3993V21.1217C23.7342 19.666 23.0787 18.4996 22.2992 18.5144C22.1043 18.5173 21.9271 18.5941 21.7617 18.7299C21.4428 18.9868 21.0796 19.1256 20.6869 19.1256C19.4674 19.1256 18.4752 17.7318 18.4752 16.0103C18.4752 14.2888 19.4644 12.8951 20.6869 12.8951C21.0826 12.8951 21.4458 13.0309 21.7617 13.2908C21.9271 13.4266 22.1043 13.5004 22.2992 13.5063C23.0817 13.5211 23.7372 12.3547 23.7638 10.899C23.7667 10.7956 23.7638 10.6952 23.7608 10.5948C23.7031 9.25524 23.5989 7.95204 23.4524 6.69177C23.3801 6.06971 23.2975 5.45811 23.2051 4.85775C23.1975 4.80835 23.1898 4.75903 23.1821 4.70979C21.3218 4.99917 19.3522 5.19996 17.297 5.28855C17.1967 5.2915 17.0963 5.2915 16.9929 5.2915C15.5372 5.26493 14.3708 4.60939 14.3855 3.82689C14.3885 3.632 14.4653 3.45778 14.6011 3.28947C14.858 2.97352 14.9968 2.60736 14.9968 2.21464C14.9968 0.992157 13.603 0 11.8815 0C10.16 0 8.76627 0.992157 8.76627 2.21464C8.76627 2.61032 8.90505 2.97352 9.16195 3.28947C9.29778 3.45483 9.37455 3.632 9.3775 3.82689C9.39522 4.60939 8.22885 5.26493 6.77309 5.2915C6.6727 5.29445 6.59592 5.29445 6.49553 5.2915C4.64892 5.20972 2.87065 5.05253 1.18389 4.80097C0.982588 4.77095 0.782585 4.73958 0.583926 4.70684C0.482223 5.35507 0.391957 6.01616 0.313749 6.68925C0.167159 7.95087 0.0629338 9.25464 0.00516748 10.5948C0.00221464 10.6952 0.00221464 10.7956 0.00221464 10.899C0.0287903 12.3547 0.717496 13.5148 1.5 13.5C1.69489 13.497 1.83889 13.4266 2.00425 13.2908C2.3202 13.0339 2.6834 12.8951 3.07613 12.8951C4.29565 12.8951 5.28781 14.2888 5.28781 16.0103C5.28781 17.7318 4.29861 19.1256 3.07613 19.1256C2.6834 19.1256 2.3202 18.9868 2.00425 18.7299C1.83889 18.5941 1.66171 18.5173 1.46683 18.5144C0.684322 18.4996 0.0287903 19.666 0.00221464 21.1217ZM2.00222 10.8784C2.00369 10.9399 2.00713 10.9987 2.01216 11.0547C2.34727 10.9514 2.70358 10.8951 3.07613 10.8951C4.50493 10.8951 5.60571 11.7143 6.26964 12.6487C6.93714 13.5881 7.28781 14.7828 7.28781 16.0103C7.28781 17.2366 6.93858 18.4312 6.27125 19.3713C5.60681 20.3072 4.50556 21.1256 3.07613 21.1256C2.70338 21.1256 2.34688 21.0692 2.01161 20.9658C2.0065 21.0268 2.00311 21.091 2.00188 21.1582L2.00168 21.1694L2.00135 21.1805C1.99969 21.237 1.99956 21.2734 2.00097 21.3268C2.05829 22.6139 2.15179 23.8565 2.28843 25.048C3.61977 24.8862 4.99529 24.773 6.40704 24.7105L6.45126 24.7085H6.79138L6.80967 24.7088C7.80063 24.727 8.79322 24.9571 9.60583 25.414C10.3372 25.8252 11.4033 26.7292 11.3772 28.2071C11.3668 28.8375 11.147 29.3544 10.8788 29.75C11.0753 29.8744 11.4211 30 11.8815 30C12.3406 30 12.6862 29.8744 12.8833 29.7496C12.6088 29.3455 12.3962 28.827 12.3858 28.2077C12.3859 28.2087 12.3859 28.2098 12.3859 28.2108M13.0272 29.6341C13.031 29.6271 13.0338 29.6238 13.0342 29.6239C13.0346 29.6239 13.0327 29.6274 13.0272 29.6341ZM10.7291 29.6255C10.7295 29.6254 10.7322 29.6287 10.736 29.6355C10.7306 29.629 10.7287 29.6256 10.7291 29.6255ZM2.00222 10.8784C2.00224 10.7877 2.00246 10.7242 2.00397 10.6658C2.05911 9.39469 2.15751 8.15963 2.29507 6.96598C3.62973 7.12262 5.00412 7.22741 6.40704 7.28954L6.42188 7.2902L6.43673 7.29064C6.57265 7.29463 6.6862 7.29474 6.82095 7.29095C7.80887 7.27131 8.79796 7.04148 9.60805 6.586C10.3393 6.17486 11.4073 5.26898 11.3771 3.78855C11.366 3.17089 11.1536 2.65366 10.8798 2.25037C11.0768 2.1256 11.4224 2 11.8815 2C12.341 2 12.6868 2.1258 12.8838 2.25068C12.6157 2.64618 12.3962 3.16291 12.3858 3.79288C12.3597 5.27105 13.4262 6.1751 14.1582 6.58633C14.9713 7.04316 15.9645 7.27306 16.9564 7.29117L16.9746 7.2915H17.003C17.1 7.29151 17.2257 7.29151 17.3558 7.28768L17.3695 7.28728L17.3832 7.28669C18.7853 7.22626 20.1499 7.11539 21.4702 6.9609C21.6082 8.15589 21.7069 9.39334 21.7621 10.6671C21.7649 10.7661 21.7656 10.8072 21.7646 10.8418L21.7643 10.8521L21.7641 10.8625C21.7629 10.9295 21.7595 10.9934 21.7544 11.0542C21.4164 10.9504 21.0587 10.8951 20.6869 10.8951C19.2575 10.8951 18.1562 11.7134 17.4918 12.6494C16.8245 13.5895 16.4752 14.7841 16.4752 16.0103C16.4752 17.2379 16.8259 18.4325 17.4934 19.372C18.1573 20.3064 19.2581 21.1256 20.6869 21.1256C21.0612 21.1256 21.4183 21.0688 21.7537 20.9652C21.7587 21.0222 21.7621 21.082 21.7638 21.1445V21.3555C21.7081 22.6329 21.6143 23.8683 21.4776 25.0537C20.1594 24.8932 18.7921 24.774 17.3832 24.7133L17.3695 24.7127L17.3423 24.7119C17.251 24.7092 17.105 24.7049 16.9467 24.709C15.958 24.7284 14.9686 24.9583 14.1582 25.4137C13.4261 25.825 12.3595 26.7292 12.3858 28.2077M13.0342 2.37614C13.0338 2.37622 13.031 2.37289 13.0271 2.36591C13.0327 2.37257 13.0346 2.37606 13.0342 2.37614ZM10.7359 2.36591C10.732 2.37289 10.7293 2.37622 10.7289 2.37614C10.7284 2.37606 10.7304 2.37257 10.7359 2.36591Z" fill="currentColor"/>
                  </svg>
                  <span class="cormorantInfant ml-1.5">{{ $product->puzzles_count }}</span>
                </div>
              @endif
                @if(!$product->getStock() && !$product->soon)
                  <div class="text-sm uppercase py-1 px-2 bg-white absolute left-0 top-2  z-10">sold out</div>
                @endif
                <div x-data="{ open: false }" :class="open ? '' : 'absolute'" class="z-10 top-2 right-2 flex flex-col space-y-0.5">
                  @if($product->refill)
                    @php($refill = $product->refill)
                    <div>
                      <div x-show="!open" @click="open = !open" class="flex-1 flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark bg-opacity-60">
                        есть refill <svg width="14" height="14" class="ml-1.5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M19.5 10C19.5 15.2239 15.2239 19.5 10 19.5C4.77614 19.5 0.5 15.2239 0.5 10C0.5 4.77614 4.77614 0.5 10 0.5C15.2239 0.5 19.5 4.77614 19.5 10Z" stroke="#2C2E35" stroke-linecap="round" stroke-linejoin="round"/>
                          <path d="M10 15V10" stroke="#2C2E35" stroke-linecap="round" stroke-linejoin="round"/>
                          <path d="M9.99609 7H10.0051" stroke="#2C2E35" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                      </div>
                      <div class="absolute z-10 p-3 w-full">
                        <div x-show="open" @click.outside="if(open) open = false" class="flex items-center text-myDark bg-white text-md leading-none border border-myDark">
                          <div class="item-square hidden sm:block" style="width: 40%;max-width: 111px;">
                            @if(isset($refill->style_page['mainImage']['image']))
                              <input type="hidden" data-id="mainImage" class="json-image"
                                     value="{{ e(json_encode($refill->style_page['mainImage']['image'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="lg:absolute lg:left-0 lg:top-0 w-full h-full block object-cover" data-img-class="block w-full">
                            @endif
                          </div>
                          <div class="py-2 px-3">
                            <div class="mb-3">Дополнительно к продукту вы можете приобрести</div>
                            <div class="font-medium">{{ $refill->name }}</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  @endif
                  @if($product->new_structure)
                    <div x-show="!open" class="flex-1 flex justify-end">
                      <div class="flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark">новый состав</div>
                    </div>
                  @endif
                    @if($product->is_new)
                      <div x-show="!open" class="flex-1 flex justify-end">
                        <div class="flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark">Новинка</div>
                      </div>
                    @endif
                    @if($product->sale)
                      <div x-show="!open" class="flex-1 flex justify-end">
                        <div class="flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark">SALE</div>
                      </div>
                    @endif

                    @if($product->tag20)
                      <div x-show="!open" class="flex-1 flex justify-end">
                        <div class="flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark cormorantInfant">-20%</div>
{{--                        <img src="{{ asset('img/bf-1125/tag-20.jpg') }}" alt="" style="width: 50px; " class="block">--}}
                      </div>
                    @endif
                    @if($product->tag30)
                      <div x-show="!open" class="flex-1 flex justify-end">
                        <div class="flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark cormorantInfant">-30%</div>
{{--                        <img src="{{ asset('img/bf-1125/tag-30.jpg') }}" alt="" style="width: 50px; " class="block">--}}
                      </div>
                    @endif
                    @if($product->tag50)
                      <div x-show="!open" class="flex-1 flex justify-end">
                        <div class="flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark cormorantInfant">-50%</div>
{{--                        <img src="{{ asset('img/bf-1125/tag-50.jpg') }}" alt="" style="width: 50px; " class="block">--}}
                      </div>
                    @endif
                  @if(!$product->getStock() && $product->soon)
                    <div class="flex-1 flex justify-end">
                      <div class="flex items-center text-myDark bg-white text-sm py-1 px-1 leading-none border border-myDark">Скоро в продаже</div>
                    </div>
                  @endif
                  @if($product->gold_coupon)
                    <div class="flex-1 flex justify-end" >
                      <img src="{{ asset('img/gold-ticket.png') }}" alt="" style="width: 27.37430168%;">
                    </div>
                  @endif
                </div>
              @if($product->category_id == 32 || $product->type_id != 1)
                {{--                          <div class="flex items-center absolute z-10 text-myGreen2 text-md" style="width: 29%;right: 6.278026906%; top:8.771929825%;">--}}
                {{--                            <img src="{{ asset('img/gtsticker.png') }}" alt="" style="width: 100%">--}}
                {{--                          </div>--}}
                @if(isset($product->cardImage['image']))
                  <div class="img product_card_preview item-square block">
                    <input type="hidden" data-id="cardImage-{{ $product->id }}" class="json-image"
                           value="{{ e(json_encode($product->cardImage['image'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover" data-img-class="block w-full object-cover">
                  </div>

                @endif
              @else
                <a href="{{ route('product.index', $product->slug) }}" class="img product_card_preview item-square block">
                  @if(isset($product->cardImage['image']))
                    <input type="hidden" data-id="cardImage-{{ $product->id }}" class="json-image"
                           value="{{ e(json_encode($product->cardImage['image'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover" data-img-class="block w-full object-cover">
                  @endif
                </a>
              @endif

            </div>
          </div>
          @if($product->style_cards)
            @foreach($product->style_cards as $key => $card)
              @if(!isset($card['card_style']))
                @continue
              @endif
              <div class="swiper-slide product-item-swiper__slide">
                <div class="product-card_item">
                  <div class="product_{{ $card['card_style'] }} relative bg-white overflow-hidden max-w-sm">
                    <div x-data="{ open: false }" :class="open ? '' : 'absolute'" class="z-10 top-2 right-2 flex flex-col space-y-0.5">
                      @if($product->gold_coupon)
                        <div class="flex-1 flex justify-end" >
                          <img src="{{ asset('img/gold-ticket.png') }}" alt="" style="width: 27.37430168%;">
                        </div>
                      @endif
                    </div>
                    <div class="card_fields z-10 absolute top-0 left-0 w-full @if($card['card_style'] == 'card-style-5') {{ $card['vertical-align'] }} @else {{ 'h-full' }} @endif" style="{{ $card['style'] ?? '' }}">
                      @if(isset($card['fields']))
                        @foreach($card['fields'] as $field)
                          <div class="cormorantGaramond lh-base product-description-item {{ $field['align'] ?? '' }} {{ $field['vertical-align'] ?? '' }}" style="{{ $field['style'] ?? '' }}">
                            {!! nl2br($field['text']) !!}
                          </div>
                        @endforeach
                      @endif
                      @if(isset($card['big-text']))
                        <div class="cormorantGaramond lh-base product-description-bigText {{ $card['big-text-vertical-align'] ?? '' }} {{ $card['big-text-align'] ?? '' }}" style="{{ $card['big-text-style'] ?? '' }}">
                          {!! nl2br($card['big-text']) !!}
                        </div>
                      @endif
                      @if(isset($card['text']))
                        <div class="cormorantGaramond lh-base product-description-item {{ $card['align'] ?? '' }} {{ $card['vertical-align'] ?? '' }}" style="{{ $card['text-style'] ?? '' }}">
                          {!! nl2br($card['text']) !!}
                        </div>
                      @endif
                      @if(isset($card['small-text']))
                        <div class="cormorantGaramond lh-base product-description-smallText {{ $card['small-text-vertical-align'] ?? '' }} {{ $card['small-text-align'] ?? '' }}" style="{{ $card['small-text-style'] ?? '' }}">
                          {!! nl2br($card['small-text']) !!}
                        </div>
                      @endif
                    </div>
                    @if(isset($card['image']))
                      <div class="img product_card_preview item-square block">

                        <input type="hidden" data-id="productImage-{{ $product->id }}-{{ $key }}{{ $getId }}" class="swiper-json-image swiper-slide-img"
                               value="{{ e(json_encode($card['image'], JSON_UNESCAPED_UNICODE)) }}" data-picture-class="block object-cover" data-img-class="block w-full">

                      </div>
                    @endif
                  </div>
                </div>
              </div>
            @endforeach
          @endif
        </div>
        <div class="product-item-swiper__pagination swiper-pagination"></div>
      </div>
    </div>

    <div class="mb-2 mt-4 md:mt-6 flex-1">
      <h3 class="break-words font-light text-lg sm:text-2xl md:text-3xl lg:text-32 sm:min-h-[52px] pr-2 lh-outline-none !leading-none">
        @if($product->category_id == 32 || $product->type_id != 1)
          {{ $product->name }}
        @else
          <a href="{{ route('product.index', $product->slug) }}">{{ $product->name }}</a>
        @endif
      </h3>
      @if($product->subtitle&&$product->subtitle!='null')
        <div class="text-myBrown d-text-body mt-2">{!! $product->subtitle !!}</div>
      @endif
      @if($product->only_pickup)
        <p class="text-myRed text-sm md:text-md lg:text-lg mt-1 !leading-none">Доступен только для самовывоза г. Волгоград</p>
      @endif
    </div>
  </div>
  <div>
    @if(getSettings('happyCoupon') && false && $product->getStock() && $product->stockStatus)
      <div class="text-base sm:mt-2 mb-4" style="color: {{ $product->stockStatus['color'] }}">
        <div class="cormorantInfant text-myDark">{!! $product->stockStatus['text'] ?? "&nbsp;" !!}</div>
        <div>
          <svg style="width: 100%;" height="4" viewBox="0 0 100% 4" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect width="100%" height="4" fill="#B2B2B2" fill-opacity="0.25"/>
            <rect width="{{ $product->stockStatus['percent'].'%' }}" height="4" fill="currentColor"/>
          </svg>
        </div>
      </div>
    @endif
    <div class="relative flex items-center justify-between mb-4 md:mb-6">
      <div class="flex space-x-2 md:space-x-4 items-center w-full justify-between pr-1">
        @if($product->price)
        <div class="">
          @if(isset($product->old_price))
            <div class="text-base sm:text-md md:text-lg cormorantInfant italic font-semibold text-myGray line-through">{{ formatPrice($product->old_price) }}</div>
          @endif
          <div class="subtitle-1 text-myBrown">{{ formatPrice($product->price) }}</div>
        </div>
        @endif
        <div class="text-sm md:text-base cormorantInfant font-medium italic opacity-50">{{ $product->volume ?: '' }}</div>
      </div>

      @if(isset($product->cardsDescription)&&$product->cardsDescription!='null')
        <a class="btn-tooltip" data-tooltip="tooltip-{{ $attributes['id'] ?? 'none' }}">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10 19C14.95 19 19 14.95 19 10C19 5.05 14.95 1 10 1C5.05 1 1 5.05 1 10C1 14.95 5.05 19 10 19Z" stroke="#B1908E" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M10 14.5V10" stroke="#B1908E" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M9.99512 7.30078H10.0032" stroke="#B1908E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </a>
      @elseif($product->category_id == 32)
        <a href="{{ route('product.index', $product->slug) }}" class="btn-tooltip">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M10 19C14.95 19 19 14.95 19 10C19 5.05 14.95 1 10 1C5.05 1 1 5.05 1 10C1 14.95 5.05 19 10 19Z" stroke="#B1908E" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M10 14.5V10" stroke="#B1908E" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M9.99512 7.30078H10.0032" stroke="#B1908E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </a>
      @endif
      @if(isset($product->cardsDescription)&&$product->cardsDescription!='null')
        <div id="tooltip-{{ $attributes['id'] ?? 'none' }}" class="tooltip hidden absolute bottom-full mb-4 shadow-md p-6 bg-white w-screen max-w-[260px] md:max-w-[386px] z-10">
          <div class="flex justify-between items-center mb-3 md:mb-6">
            <h4 class="headline-4">о продукте</h4>
            <button class="close-tooltip">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M10.0003 18.3327C14.5837 18.3327 18.3337 14.5827 18.3337 9.99935C18.3337 5.41602 14.5837 1.66602 10.0003 1.66602C5.41699 1.66602 1.66699 5.41602 1.66699 9.99935C1.66699 14.5827 5.41699 18.3327 10.0003 18.3327Z" stroke="#2C2E35" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M7.6416 12.3592L12.3583 7.64258" stroke="#2C2E35" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M12.3583 12.3592L7.6416 7.64258" stroke="#2C2E35" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </button>
          </div>
          <div class="mb-3 md:mb-6 subtitle-2">
            {!! $product->cardsDescription !!}
          </div>
          <div class="flex flex-col justify-between space-y-3 mb-3 md:mb-6  subtitle-2">
            @if(isset($product->cardsDescriptionIcons))
              @foreach($product->cardsDescriptionIcons as $icon)
                <div class="flex items-center">
                  <div class="w-5 h-5 mr-2">
                    <img src="{{ $icon['icon'] }}" alt="{{ $icon['text'] }}" class="w-[18px] h-[18px]">
                  </div>
                  <div>{{ $icon['text'] }}</div>
                </div>
              @endforeach
            @endif

          </div>
          @if($product->category_id != 32 && $product->type_id == 1)
            <x-public.primary-button href="{{ route('product.index', $product->slug) }}" class="w-full h-8">Читать подробнее</x-public.primary-button>
          @endif
          <div class="absolute tooltip-arrow w-3 h-3 -bottom-1 right-1/2 transform translate-x-1/2 rotate-45 bg-white shadow-sm"></div>
        </div>
      @endif
    </div>
    @if($product->getStock())
      @if(isset($product->product_options['productSize'])&&!empty($product->product_options['productSize']))
        <x-public.primary-button href="{{ route('product.index', $product->slug) }}" class="w-full"><span class="text-base sm:text-xl whitespace-nowrap">Выбрать размер</span></x-public.primary-button>
      @else
        @if($product->preorder)
          <x-public.primary-button href="javascript:;" data-fancybox="preorder-{{ $product->id }}" data-src="#preorder-{{ $product->id }}" class="w-full !px-2 text-center"><span class="text-base sm:text-xl whitespace-nowrap">Оформить предзаказ</span></x-public.primary-button>
          <div id="preorder-{{ $product->id }}" class="d-text-body m-text-body max-w-3xl" style="display: none;">
            <div class="p-2 sm:p-4">
              <div class="mb-4">
                <h3 class="d-headline-4 m-headline-3">Внимание</h3>
                <p>Ожидаемая отправка с 5 по 17 декабря<br/>После отгрузки товара, статус вашего заказа изменится в личном кабинете</p>
              </div>
              <x-public.primary-button href="#" class="w-full toCart !px-2 text-center" data-id="{{ $product->id }}">Подтверждаю заказ</x-public.primary-button>
            </div>

          </div>
        @else
          <x-public.primary-button href="#" class="w-full toCart !px-2 text-center" data-id="{{ $product->id }}"><span class="text-base sm:text-xl whitespace-nowrap">В корзину</span></x-public.primary-button>
        @endif
      @endif
    @else
      <div class="text-center text-xl bg-gray-200 text-gray-500 h-11 flex justify-center items-center">
        @if(auth()->check())
          @if(!$product->notification)
            <button onclick="window.productNotification(this, '{{ $product->slug }}', 'set')">Узнать о поступлении</button>
          @else
            <button onclick="window.productNotification(this, '{{ $product->slug }}', 'remove')">Сообщим о поступлении</button>
          @endif
        @else
          <a href="javascript:;" class="outline-none" data-src="#authForm" onclick="window.productNotificationBeforeAuth('{{ $product->slug }}')" data-fancybox-no-close-btn>Узнать о поступлении</a>
        @endif
      </div>
    @endif
  </div>
</div>
