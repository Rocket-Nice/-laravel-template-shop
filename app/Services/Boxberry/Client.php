...
namespace App\Services\Boxberry;

use App\Services\Boxberry\Entity\CalculateParams;
use App\Services\Boxberry\Entity\Intake;
use App\Services\Boxberry\Entity\Order;
use App\Services\Boxberry\Entity\TariffInfo;
use App\Services\Boxberry\Exception\BoxBerryException;
...
class Client implements LoggerAwareInterface
{
    ...
    private function callApi($type, $method, $params = [])
    {
        ...
        if ($response->getStatusCode() != 200)
            throw new BoxBerryException('Неверный код ответа от сервера Яндекс при вызове метода ' . $method . ': ' . $response->getStatusCode(), $response->getStatusCode(), $json, $request);

        $respBB = json_decode($json, true);

        if (empty($respBB))
            throw new BoxBerryException('От сервера Яндекс при вызове метода ' . $method . ' пришел пустой ответ', $response->getStatusCode(), $json, $request);

        if (!empty($respBB['err']))
            throw new BoxBerryException('От сервера Яндекс при вызове метода ' . $method . ' получена ошибка: ' . $respBB['err'], $response->getStatusCode(), $json, $request);


        if (!empty($respBB[0]['err']))
            throw new BoxBerryException('От сервера Яндекс при вызове метода ' . $method . ' получена ошибка: ' . $respBB[0]['err'], $response->getStatusCode(), $json, $request);
    ...
    public function createIntake($intake)
    {
        ...
        if (empty($response['message']))
            throw new BoxBerryException('От сервера Яндекс не пришел номер заявки!');
