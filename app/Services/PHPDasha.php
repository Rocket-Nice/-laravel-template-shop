...
class PHPDasha {
  ...
  private function sendData($method = null, $params) {
    ...
    if (!$final)
      throw new Exception('Получены неверные данные, пожалуйста, убедитесь, что запрашиваемый метод API существует');
    if(!isset($final['msg']['err_code'])){
      Log::debug(print_r($final, true));
    }
    if ($final['msg']['err_code'] == '0') {
      return $final['data'];
    } else {
  ...
  */
  ...
  // добавлено temasite
  //lists.add_member_batch - Добавляем подписчика в базу
  /*
  required: list_id, batch
  JSON-массив добавляемых подписчиков: [{"email":"test1@dashamail.ru","merge_1":"Ваня","gender":"m","region":17,"state":"active"},{"email":"test2@inbox.ru","merge_1":"Петя","state":"inactive"}].
  Каждый JSON-объект подписчика может содержать следующие параметры: email (email-адрес подписчика), merge_xxx (значения дополнительных полей), state (active – активен, inactive - неактивен), gender (пол подписчика, возможные значения: m - мужчина, f - женщина, n - неизвестно), region (номер региона подписчика).
  optional: merge_1, merge_2..., update...
  see: http://dashamail.ru/api_details.php?method=lists.add_member
  */
  public function lists_add_member_batch($list_id = null, $batch = [], $params = array()) {
    if (is_null($list_id) || empty($batch))
      return $this->getError('3');

    $batch_json = [];
    foreach($batch as $key => $email_data){
      $email = $this->checkEmail($email_data['email']);
      if ($email !== false)
        $batch_json[] = $email_data;
    }
    if(empty($batch_json)){
      return $this->getError('6');
    }
    $required = array('list_id' => $list_id, 'batch' => json_encode($batch_json));
    $params = array_merge($required, $params);
