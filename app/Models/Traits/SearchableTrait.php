...
//            $this->filterQueryWithMaximumRelevance($query, $this->getRelevanceField());
        }
  ...
  protected function filterQueryWithMaximumRelevance(Builder $query, $comparator)
  {
    $query->havingRaw("$comparator = (SELECT MAX($comparator) FROM ({$query->toSql()}) as temp)", $query->getBindings());
  }
    /**
